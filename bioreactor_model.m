...
f = @(t,x) [N_i * ((mu_m * x(2)) / (K_s + x(2))) - x(1) * (D + kappa);
    ...
    beta * x(4) - alpha * x(3) * (1 - x(4))/(delta * x(1));
    -(beta * x(4) - alpha * x(3) * (1 - x(4))/(delta * x(1))) - D*x(4)];
